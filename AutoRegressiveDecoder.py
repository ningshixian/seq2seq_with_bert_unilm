class AutoRegressiveDecoder(object):
    ...
    def random_sample(
        ...
        返回：n个解码序列组成的list。
        解读：https://zhuanlan.zhihu.com/p/268529417
        """
        inputs = [np.array([i]) for i in inputs]
        ...
        for step in range(self.maxlen):
            ...
            )  # 预测第一个字的所有可能的概率，shape=(1, 13584)
            probas /= probas.sum(axis=1, keepdims=True)  # 确保归一化
            if step == 0:  # 第1步预测后将结果重复n次
                probas = np.repeat(probas, n, axis=0) # shape=(n, 13584)
            ...
            #     probas /= probas.sum(axis=1, keepdims=True)  # 重新归一化
            sample_func = lambda p: np.random.choice(len(p), p=p)  # 定义采样函数：按照概率大小进行随机采样
            ...
            output_ids = np.concatenate([output_ids, sample_ids], 1)  # 更新输出:加到token_ids后面的
            is_end = output_ids[:, -1] == self.end_id  # 标记是否以end标记结束
            end_counts = (output_ids == self.end_id).sum(1)  # 统计出现的end标记
            if output_ids.shape[1] >= self.minlen:  # 最短长度判断
